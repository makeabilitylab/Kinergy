...
namespace Kinergy
{
    namespace Geom
    {
        public class Spiral
        {
            ...
            private Curve GenerateSpiralCurve()
            {
                FixParameter();
            ...
            private Brep GenerateSpiralBrep()
            {
                spiralCurve = GenerateSpiralCurve();
                Plane basePlane = new Plane(center, direction);
                Vector3d X = basePlane.XAxis;
                ...
                Brep b = Brep.CreateFromSweep(spiralCurve, outline.ToNurbsCurve(), true, 0.00000001)[0];
                return b;
            ...
            public Curve GetSpiralCurve()
            {
                return spiralCurve;
            }
            public Brep GetSpiralBrep()
            {
                return spiralBrep;
            }
            public Brep SetPosition(Point3d Center, Vector3d Direction)
            {
                center = Center;
                direction = Direction;
                spiralBrep = GenerateSpiralBrep();
            ...
            public Brep SetParameter(double R = 0, double r = 0, double ThicknessX = 0, double ThicknessY = 0, int RoundNum = 0)
            {
                if (R > 0)
                { outerRadius = R; }
                if (r > 0)
