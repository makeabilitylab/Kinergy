...
namespace HumanUIforKinergy.KinergyUtilities
{
    public class ModelPreprocessWithoutInputDirection : GH_Component
    {
        ...
        /// <summary>
        /// Initializes a new instance of the ModelPreprocessWithoutInputDirection class.
        /// </summary>
        public ModelPreprocessWithoutInputDirection()
          : base("ModelPreprocessWithoutInputDirection", "MPWID",
              "Preprocess model without given direction to generate 2 surfaces. User move the surfaces in rhino interface to partition model and generate region of interest",
              "Kinergy", "Utilities")
        {
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            Arrow ax1 = new Arrow(new Vector3d(1, 1, 0), box.PointAt(1, 0, 0.5), arrowScale);
            Arrow ax2 = new Arrow(new Vector3d(-1, 1, 0), box.PointAt(0, 0, 0.5),arrowScale);
            Arrow ay1 = new Arrow(new Vector3d(0, 1, 1), box.PointAt(0.5, 1, 0), arrowScale);
            Arrow ay2 = new Arrow(new Vector3d(0, -1, 1), box.PointAt(0.5, 0, 0), arrowScale);
            Arrow az1 = new Arrow(new Vector3d(-1, 0, 1), box.PointAt(1, 0.5, 1), arrowScale);
            Arrow az2 = new Arrow(new Vector3d(-1, 0, -1), box.PointAt(1, 0.5, 0), arrowScale);
        ...
        private void Gp_SelectionMouseDown(object sender, Rhino.Input.Custom.GetPointMouseEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    v.Transform(Transform.Rotation(Math.PI / 12, Vector3d.ZAxis, Point3d.Origin));
                    generated = false;
                }
                else if (min == disX2)
                {
                ...
                {
                    v.Transform(Transform.Rotation(Math.PI / 12, Vector3d.XAxis, Point3d.Origin));
                    generated = false;
                }
                else if (min == disY2)
                {
                ...
                {
                    v.Transform(Transform.Rotation(Math.PI / 12, Vector3d.YAxis, Point3d.Origin));
                    generated = false;
                }
                else if (min == disZ2)
                {
        ...
        {
            get { return new Guid("de1cd30f-17d4-4775-98de-95e182069928"); }
        }
    }
}