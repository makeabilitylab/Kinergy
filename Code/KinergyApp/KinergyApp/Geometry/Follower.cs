    ...
    public class Follower:Component
    {
        ...
        public Follower(int type,Entity modelToFixTo, Cam c,Point3d Point1,Point3d Point2, double rodRadius, double thick=5)
        {
            ...
            if(type==2)
            {
                ...
                Curve capsule = GeometryMethods.Capsule(new Plane(midPoint, c.Normal), radius, radius, new Plane(midPoint, c.Normal, new Vector3d(midPoint) - new Vector3d(c.CenterPoint)).Normal);
        ...
        public void Follow()
        {
            ...
            if(FollowerType==1)
            {
                ...
                bool intersect = GeometryMethods.IfBrepsIntersect(f, cam.GetModelinWorldCoordinate());
                if (intersect)//go upwards an keep trying until no intersection
                {
                    ...
                    do
                    {
                        ...
                        moveValue -= step;
                        f.Transform(m);
                        intersect = GeometryMethods.IfBrepsIntersect(f, cam.GetModelinWorldCoordinate());
                        counter++;
                        if(counter>200)
